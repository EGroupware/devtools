...
function widget_browser(_et2)
{
	...
	this.et2.setArrayMgrs(_et2._createArrayManagers(_data));

	// Set up UI
	this.list_div = jQuery(this.et2.getWidgetById('widget_list').getDOMNode());
	this.attrs_div = jQuery(this.et2.getWidgetById('widget_attrs').getDOMNode());
...
widget_browser.prototype._init_list = function()
{
	...
	let attribute_table = jQuery(document.createElement('table'));
...
widget_browser.prototype.select_widget = function(e,f)
{
	...
	if(this.widget)
	{
		this.et2.removeChild(this.widget);
		this.widget.destroy();
		this.widget = null;
	}
	...
	window.wb_widget = this.widget = et2_createWidget(type, attrs, this.et2.getWidgetById('widget_container'));
...
widget_browser.prototype._dtd_builder = function()
{
	...
	for (var widget_type in et2_registry)
	{
		...
		if (!widget_type.match(/nextmatch/,'i'))
		{
			...
			this.widgets[i] = et2_createWidget(widget_type ,attrs, this.et2)
